...
import com.flybirds.common.util.http.HttpUtils;
import com.flybirds.system.sysSocial.dto.SocialUserBindReqDTO;
import com.flybirds.system.sysSocial.entity.SysSocialUser;
import com.flybirds.system.sysSocial.mapper.SocialAuthUserRedisMapper;
import com.flybirds.system.sysSocial.mapper.SocialUserMapper;
...
import static com.flybirds.common.util.json.jackson.JsonUtil.toJsonString;
...
public class SocialUserServiceImpl implements SocialUserService {
    ...
    @Resource
    private SocialAuthUserRedisMapper authUserRedisMapper;
    ...
    public AuthUser getAuthUser(Integer type, String code, String state) {
        ...
        // 从缓存中获取
        AuthUser authUser = authUserRedisMapper.get(type, authCallback);
        if (authUser != null) {
            return authUser;
        }
        ...
        authUserRedisMapper.set(type, authCallback, authUser);
        return authUser;
    }
     ...
     * @return 社交用户列表
     */
    private List<SysSocialUser> getAllSocialUserList(Integer type, String unionId, Integer userType) {
    ...
    @Override
    public List<SysSocialUser> getSocialUserList(Long userId, Integer userType) {
    ...
    protected void bindSocialUser(Long userId, Integer userType, Integer type, AuthUser authUser) {
        ...
        List<SysSocialUser> socialUsers = this.getAllSocialUserList(type, unionId, userType);
        ...
        socialUsers.forEach(socialUser -> {
            ...
            SysSocialUser sysSocialUser = new SysSocialUser().setId(socialUser.getId()).setUserId(userId);
            sysSocialUser.builderUpdateBaseDo(userId,socialUser.getNickname());
            socialUserMapper.updateById(sysSocialUser);
        });

        // 逻辑三：如果 authUser 不存在于 socialUsers 中，则进行新增；否则，进行更新
        SysSocialUser socialUser = CollUtil.findOneByField(socialUsers, "openid", authUser.getUuid());
        SysSocialUser saveSocialUser = SysSocialUser.builder() // 新增和更新的通用属性
        ...
        if (socialUser == null) {
            saveSocialUser.setUserId(userId).setUserType(userType)
                    ...
                    .builderInsertBaseDo(userId,authUser.getNickname());

            socialUserMapper.insert(saveSocialUser);
        } else {
            saveSocialUser.setId(socialUser.getId())
                    .builderUpdateBaseDo(userId,authUser.getNickname());
    ...
    public void unbindSocialUser(Long userId, Integer userType, Integer type, String unionId) {
        ...
        // 解绑
        socialUserMapper.deleteBatchIds(CollectionUtils.convertSet(socialUsers, SysSocialUser::getId));
    }
    ...
    public Long getBindUserId(Integer userType, Integer type, String code, String state) {
        ...
        String unionId = getAuthUserUnionId(authUser);
        List<SysSocialUser> socialUsers = getAllSocialUserList(type, unionId, userType);
        if (CollUtil.isEmpty(socialUsers)) {
    ...
    public void unbindOldSocialUser(Long userId, Integer userType, Integer type, String newUnionId) {
        List<Integer> types = SocialTypeEnum.getRelationTypes(type);
        List<SysSocialUser> oldSocialUsers = socialUserMapper.selectListByTypeAndUserId(userType, types, userId);
        ...
        // 解绑
        socialUserMapper.deleteBatchIds(CollectionUtils.convertSet(oldSocialUsers, SysSocialUser::getId));
    }
