package com.flybirds.system.sysMenu.controller;
...
@RestController
@RequestMapping("/menu")
public class SysMenuController extends MpBaseController
{
    @Autowired
    ...
    {
        if (Constant.User.NOT_UNIQUE.equals(menuService.checkMenuNameUnique(menu)))
        {
            return AjaxResult.error("新增菜单'" + menu.getMenuName() + "'失败，菜单名称已存在");
        }
        else if (Constant.User.YES_FRAME.equals(menu.getIsFrame())
                && !StringUtils.startsWithAny(menu.getPath(), Constant.HTTP, Constant.HTTPS))
        {
            return AjaxResult.error("新增菜单'" + menu.getMenuName() + "'失败，地址必须以http(s)://开头");
        }
        menu.setCreateName(SecurityUtils.getUsername());
    ...
    public AjaxResult edit(@Validated @RequestBody SysMenu menu)
    {
        ...
        menu.setUpdateName(SecurityUtils.getUsername());
