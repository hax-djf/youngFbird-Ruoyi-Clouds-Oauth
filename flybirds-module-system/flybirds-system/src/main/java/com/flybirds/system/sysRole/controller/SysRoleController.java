package com.flybirds.system.sysRole.controller;
...
@RestController
@RequestMapping("/role")
public class SysRoleController extends MpBaseController
{
    @Autowired
    ...
    @GetMapping("/list")
    public MpTableDataInfo list(SysRole role)
    {
        mpStartPage();
    ...
    {
        List<SysRole> list = roleService.selectRoleList(role);
        POIExcelUtil<SysRole> util = new POIExcelUtil<SysRole>(SysRole.class);
    ...
    {
        if (Constant.User.NOT_UNIQUE.equals(roleService.checkRoleNameUnique(role)))
        {
            return AjaxResult.error("新增角色'" + role.getRoleName() + "'失败，角色名称已存在");
        }
        else if (Constant.User.NOT_UNIQUE.equals(roleService.checkRoleKeyUnique(role)))
        {
            return AjaxResult.error("新增角色'" + role.getRoleName() + "'失败，角色权限已存在");
        }
        role.setCreateName(SecurityUtils.getUsername());
    ...
    public AjaxResult edit(@Validated @RequestBody SysRole role)
    {
        ...
        role.setUpdateName(SecurityUtils.getUsername());
    ...
    public MpTableDataInfo allocatedList(SysUser user)
    {
        mpStartPage();
    ...
    public MpTableDataInfo unallocatedList(SysUser user)
    {
        mpStartPage();
