package com.flybirds.smsprovider.client.impl.aliyun;
...
public class AliyunSmsClient extends AbstractSmsClient {
    ...
    protected SmsResult<SmsSendRespDTO> doSendSms(Long sendLogId, String mobile,
        ...
        request.setTemplateParam(JsonUtil.toJsonString(MapUtils.convertMap(templateParams)));
        //阿里云的帮助文档说明 外部流水扩展字段
    ...
    protected List<SmsReceiveRespDTO> doParseSmsReceiveStatus(String text) throws Throwable {
        List<SmsReceiveStatus> statuses = JsonUtil.parseArray(text, SmsReceiveStatus.class);
    ...
    @VisibleForTesting
    <T extends AcsResponse, R> SmsResult<R> invoke(AcsRequest<T> request, Function<T, R> responseConsumer) {
        try {
            ...
            // 拼接结果
            return SmsResult.build(code, message, requestId, data, codeMapping);
        } catch (ClientException ex) {
            return SmsResult.build(ex.getErrCode(), formatResultMsg(ex), ex.getRequestId(), null, codeMapping);
        }
    }
